...
- FF-07: write the presentation language into the PPTX through core `resolveScriptFonts()` (FF-18). Runs, end-of-paragraph and default run properties carry the resolved OOXML `lang` (for example `ja-JP`, `ar-SA` or `en-GB`) instead of a fixed `en-US`; `altLang` is not written. Theme major/minor `ea`/`cs` name the resolved East Asian and complex-script fonts instead of the vendored empty values; in Latin-script decks they repeat the chosen heading/body family. The theme's per-script entry for the language's own script names the resolver's supplement. Run `ea`/`cs` name the language's script font where one is chosen, and right-to-left languages set `rtl="1"` on every slide and notes paragraph and on the master default levels, with alignment unchanged. `fromPptx` maps run `lang` back to `language` (a catalog id when it round-trips, else the tag) and reports `mixed-run-languages`, `language-ambiguous`, `language-uncatalogued`, `rtl-language-mismatch` and `script-font-not-imported`. The rewrite happens during package normalization, and the vendored bytes are unchanged. With a core that has no resolver (published 0.11.0), output stays byte-identical to before and a document that names a language gets `language-export-unavailable`. Across the 126 bundled examples, all decks change: 52 en-US or language-less decks only in `ppt/theme/theme1.xml` (major/minor `ea`/`cs`), 71 en-GB/en-AU/en-CA/en-IN/de/pt decks also in their `lang` attributes, one Japanese deck also in run `ea` and the `Jpan` supplement, and two Arabic decks also in run `cs`, the `Arab` supplement and `rtl`. The corpus grows by 632 bytes in total.
