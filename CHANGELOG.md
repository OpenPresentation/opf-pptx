...
- FF-22: every classic chart type the core catalog keeps exports its exact native construct and imports back to the same id. New `src/chart-types.js` maps the 26 kept core chart ids to their Aspose.Slides `ChartType` and Office construct and the 50 deprecated ids to their replacement (other ids keep the previous substring heuristic). Column/bar charts now write `percentStacked` for the 100% types (previously `stacked`); line charts write the required `c:grouping` (standard or stacked) and markers only for the marker types (`line`, `stacked-line-3x`, `radar` have none); area charts write standard, stacked or percentStacked grouping (stacking was previously dropped); 100% types format the value axis `0%`; `filled-radar` and `radar-with-markers` write their `radarStyle` (previously always standard); `scatter` plots X from the first series column and Y from the rest with markers only (previously the category rows were misread as X); pie and doughnut plot the first series. Line and area series no longer carry the bar-only `c:invertIfNegative`. Import recognises the grouping, markers and radar style (and 3D/of-pie variants) and returns the kept id, e.g. `stacked-line-with-markers-3x`; scatter import reads `c:xVal`/`c:yVal`. `test/chart-types.mjs` checks all 19 classic types, the 50 deprecated aliases and legacy ids. Chartex types are unchanged in this slice.
