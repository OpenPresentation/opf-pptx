...
- FF-34: export generated `socials` furniture as native text. Each line is linked (`a:hlinkClick`) to its full profile URL, with the furniture colour kept (`hlinkClr=tx`) and no underline, matching the preview. The furniture manifest records each line's platform id and the `https://` scheme stripped from the display, never its words. Re-import rebuilds `organization.socials` from the current lines, with handles becoming canonical profile URLs, and keeps `socials: true`. Disagreeing, cleared or unowned lines (no repeated organization name) fall back to ordinary text with a specific `invalid-furniture-provenance` diagnostic. SocialPlatforms records resolve in the same order as the opf-render preview: inline records, the document `catalogs.socialPlatforms.source` (new `options.catalogSources`), injected records (new `options.catalogs`), then the bundled catalog. Re-imported platform keys are checked against the Socials schema pattern itself. With FF-32 document provenance, an unedited profile line restores the authored value (a handle stays a handle); an edited line keeps its new URL, and disagreeing or unowned lines fall back to the stored organization. Decks without `socials: true` export byte-identically.
