...
Unfinished prepared shaping work is preserved in the [September 15 roadmap](docs/roadmap-shaping-20260915.md); it is not part of the published runtime. Native Office Header/Footer objects for OPF furniture remain [core issue 87](https://github.com/OpenPresentation/opf/issues/87); 0.8.1 tagged slide shapes are not that work.
...
Version 0.8.1 exports core's accepted header/footer text and picture geometry as ordinary slide shapes. Dedicated `OPF_FURNITURE_V1` tags describe roles, line boundaries, inactive flags and global/local scope. A slide manifest represents empty or disabled definitions without adding a visible shape. Tags contain no original text, image bytes or alt text. Reimport reads those values from current native shapes; complete groups recover literal dates, section/organization metadata and page-number intent when the current number matches the current slide position. Vendor `p:hf` flags stay off (`sldNum="0" hdr="0" ftr="0" dt="0"`). Notes-master placeholders remain generator defaults. This is **not** Office Insert → Header & Footer, notes-master date/slide-number/footer fields, or native `p:hf` objects.
...
Native Office Header/Footer objects remain deferred [core issue 87](https://github.com/OpenPresentation/opf/issues/87) work. OPF shared headers/footers **must** compile into those objects (Insert → Header & Footer, notes master, `p:hf` date/slide-number/footer). Tagged slide shapes plus provenance are an interim portable contract, not the intended PowerPoint end state. Do not treat furniture as done for PowerPoint. Visual corpus gates and native COM acceptance stay separate.
