...
- Conditional table styles, merged-cell geometry, cell fills/borders/alignment, unsupported text fills/colors, and internal hyperlink actions are not fully reconstructed. `onDiagnostic` reports unsupported table style references, merges, fonts/colors/fills and links with native frame/cell paths. Table paths use the native graphic-frame and row indexes, including a header row. The current shared table layout caps row height at 54 scaled pixels; preserved multiline cells can overflow and report `text-overflow`. These checks establish native XML conversion, not visual parity with PowerPoint.
