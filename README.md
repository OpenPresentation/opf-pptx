...
- Theme major/minor `a:ea`/`a:cs` name the resolved heading/body East Asian and complex-script fonts instead of the vendored empty values. In a Latin-script deck they repeat the chosen heading/body family. This fills the slots for correctness. Native evidence shows it does not by itself remove the nameless and Aptos entries PowerPoint lists at open (FF-05).
...
- Right-to-left languages set `rtl="1"` on every slide and notes paragraph, and on the master, layout and presentation default paragraph levels. Alignment is unchanged: `algn` stays the composed absolute alignment (left stays `l`), so native line placement keeps matching the renderer's geometry. Right-aligning RTL text by default is a composition decision in core; until then, set `contentAlignment` or `titleAlignment` to `right`.
