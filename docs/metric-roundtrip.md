...
Comparison checks all 144 original/saved/edited imports, native source-character bounds, tab positions, raster containment and intersections between isolated field pixel masks. It writes all evidence before failing a fidelity gate; successful imports alone do not pass that gate. Current Node 20/24 observations have no character-bound overruns or inter-field mask collisions, but retain eight tab outliers (maximum 0.067383pt against the 0.02pt gate) and one raster-edge outlier. Font, baseline and raster equivalence remain unproven. See the core evidence rather than treating these numbers as universal tolerances.
