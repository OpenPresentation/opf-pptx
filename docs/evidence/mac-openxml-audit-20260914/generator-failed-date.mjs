...
if (!process.argv[2]) throw new Error('Usage: node test/openxml/generate.mjs NEW_OUTPUT_DIRECTORY');
...
for (const {id, source, options = {}} of cases) {
  ...
  const reordered = zipSync(Object.fromEntries(Object.entries(entries).sort(([a], [b]) => a.localeCompare(b)).map(([name, value]) => [name, [value, {mtime: new Date('1980-01-01T00:00:00Z')}]])), {level: 6});
...
await writeFile(path.join(root, 'manifest.json'), JSON.stringify({node: process.version, verifierSha256: hash(await readFile(new URL(import.meta.url))), cases: manifest}, null, 2) + '\n');
