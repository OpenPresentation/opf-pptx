...
Invocation safety is a PowerShell AST check. `Get-MixedEditNumericLiteralValue` unwraps `ParenExpressionAst`, then accepts a constant or a unary minus. `-PureRegression` evaluates `Write-MixedEditStage` before `Invoke-MixedEditCom`, points `$script:stageFile`, `$script:progressFile`, and `$script:sequence` at a temp directory, and deletes that directory in `finally`. Throw strings and comments are not invocations. The AST check also rejects `Invoke-Expression`, `iex` and `Add-Type` anywhere except the pure regression's exact re-evaluation of its two helper definitions. It rejects `Invoke-Command`, `[scriptblock]::Create`, `InvokeScript`, `NewScriptBlock`, `$ExecutionContext` and dynamic member names. It also rejects any non-literal `&` or `.` invocation except `& $Operation` in `Invoke-MixedEditCom` and the script-level `. $processSnapshot` and `. $fontHelperSnapshot`.

The directory audit applies an offline source policy to the verifier snapshot. It uses the shared PowerShell lexer in `test/powershell-scan.mjs`, which blanks comments, strings and here-strings while preserving offsets, and fails closed on syntax it does not model. The policy rejects `.Quit()`, `.Kill()`, process-kill commands and the same dynamic code outside those exemptions. It requires exactly one `SaveAs($savedPath,24,0)` and no other `SaveAs`. Member assignments may target only local report roots or the one documented COM setter, the edited run's `Text` (`MIXED_EDIT_COM_SETTERS`).
