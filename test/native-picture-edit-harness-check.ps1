...
if($ScriptPath -eq '') { $ScriptPath=Join-Path $PSScriptRoot 'native-picture-edit.ps1' }
if(Test-Path -LiteralPath $ReportPath) { throw 'Preserve the existing report and select a fresh ReportPath' }
$tokens=$null
$parseErrors=$null
$ast=[System.Management.Automation.Language.Parser]::ParseFile($ScriptPath,[ref]$tokens,[ref]$parseErrors)
if($parseErrors.Count -ne 0) { throw ($parseErrors | Out-String) }
$functionAst=$ast.Find({param($node) $node -is [System.Management.Automation.Language.FunctionDefinitionAst] -and $node.Name -eq 'Invoke-EditCom'},$true)
...
$observedEmpty=Invoke-EditCom 'empty' { return ,$empty }
$observedPopulated=Invoke-EditCom 'populated' { return ,$populated }
...
$failureCaught=$false
try { Invoke-EditCom 'controlled-error' { throw 'Deliberate non-Office failure' } } catch { $failureCaught=$true }
...
try { Invoke-EditCom 'forbidden-followup' { $script:unexpectedCall=$true } } catch { }
