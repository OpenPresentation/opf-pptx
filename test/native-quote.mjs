...
const footer = 'A reviewer - Recorded interview';
if(mode==='generate') {
  ...
  for(const dimensions of [{width:1280,height:720},{width:540,height:960}]) {
    ...
    decks.push({id,...dimensions,slides:document.slides.length,hashes});
  }
  await write('generation.json',{runtime,fontHashes,fontSubstitutions:fonts.substitutions,decks,footer,scope:'Local Calibri; eight long quotes on wide/portrait slides. Intermediate weights resolve to the available regular/bold faces. Native glyph bounds and editable save/reimport are separate from raster-equivalence claims.'});
  console.log('Generated eight native quote cases with controlled local Calibri.');
} else {
  ...
  for(const record of generation.decks) {
    ...
    for(const slide of observed.slides) {
      assert.ok(slide.bodyLines>1 && slide.bodyBottom<=slide.footerTop,'Actual native quote glyphs must remain above the footer');
    ...
    for(const suffix of ['', '-native-saved','-native-edited']) {
      ...
      for(const [index,slide] of restored.slides.entries()) {
        assert.ok(JSON.stringify(slide).includes(footer),'Footer survives native save/reimport');
    ...
    contacts.push({file,sha256:hash(await readFile(path.join(output,file))),rows:[12,16,20,24],columns:['renderer','native PowerPoint']});
  }
  assert.equal(comparisons.length,8);
  await write('comparison.json',{...generation,native,comparisons,imports,contacts});
  console.log('Native quote checks passed: eight glyph separations, eight editable save/reopens, six valid deck imports with all footers and native edits retained. Raster differences are observations, not equivalence thresholds.');
}
